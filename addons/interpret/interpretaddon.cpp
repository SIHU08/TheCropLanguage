    ...
    for (const auto &item: function.codes) {
        if (item.type == EXECUTE_FUNCTION) {
            ...
            vector<string> arguments = splitWithRegex(
                    st, R"(==|!=|>=|<=|>|<(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$))");
            Variable leftTerm = getVariableOrTemporaryVariable(arguments[0], variableMap),
                    rightTerm = getVariableOrTemporaryVariable(arguments[1], variableMap);
            ...
            if (isTrue) {
                callFunction(Function(Type(), genRandomString(8), item.ifCodeBlock));
            } else {
                callFunction(Function(Type(), genRandomString(8), item.elseCodeBlock));
            }
        }
    }
}
